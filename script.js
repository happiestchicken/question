...
const questions = Array(
    ...
    "What does OOP stand for and what is it?",
    "From OOP, what are classes, objects, and class constructors?",
    "What is inheritance? (from OOP)",
    "What is polymorphism? (from OOP)",
    "What is abstraction? (from OOP)",
    "What is encapsulation? (from OOP)",
...
const answers = Array(
    ...
    "An array is a collection of similar-type data items stored at contiguous memory locations. It allows for data to be accessed only by its indexing number(s).",
    "A linear data structure in which the elements are NOT necessarily stored in a contiguous manner. The data elements are instead linked via 'pointers' which index the location of the following element in the sequence.",
    ...
    "A class is a collection of objects which the user defines. Classes are types of data structures that make the data easier to sort, find, manipulate, etc. Objects are instances of a class created with specifically defined data. Class constructors assign values to the data members of the class when an object of the class is created. For more information on OOP, as well as to find examples, visit: https://www.analyticsvidhya.com/blog/2020/09/object-oriented-programming/#:~:text=Now%2C%20there%20are%20four%20fundamental,in%20order%20to%20understand%20OOPs.",
    "A concept in which the 'child' class will inherit all the public and protected properties and methods/behaviors from the 'parent' class. In addition, it can have its own properties and methods.",
    "Polymorphism describes situations in which something occurs in several different forms. It describes the concept that you can access objects of different types through the same interface and each type can provide its own independent implementation of this interface.",
    "Abstraction is a concept where only necessary attributes of a larger data pool are shown to the user. Unnecessary data is hidden from the users to reduce complexity.",
    "Encapsulation refers to the bunching of data and code into a single unit. A common example of encapsulation are classes in OOP (object-oriented programming) languages. Classes contain both variables (data) and behaviours (functions/methods).",
    ...
    "A graph is a type of non-linear data structure that is used to store data in the form of nodes and edges/arcs. Fore more information, visit: https://afteracademy.com/blog/introduction-to-graph-in-programming#:~:text=A%20graph%20is%20a%20type,edges%20in%20the%20Graph%20G.",
    "A deque is a double-ended queue in which data elements can be added or removed from either end (the front/head and the back/tail).",
    "A stack follows the last in, first out principle (LIFO) and so to access any of the data, you must start at the top of the stack. Arrays do not follow any specific order. Instead, data can be accessed by refering to the index of the element.",
    );
